...
const FIRE_DRILL_QUESTIONS=[
  ...
  {q:"Were the required evacuation items taken?",note:"Nursery phone and tablet with child/staff/visitor attendance information/registers."},
...
function renderOutingRisks(){
  ...
  box.innerHTML=outingRisks.map((r,i)=>`<div class="outing-risk-row">
    ...
    <div class="choice-row">
      <button type="button" class="choice ${r.applicable===true?"selected applicable":""}" data-outing-risk="${i}" data-applicable="yes">Applicable</button>
...
async function makeOutingPdfBlob(r){
  ...
  // Keep the full checklist together where possible and use only standard Helvetica characters.
  // The Unicode tick previously caused jsPDF to render these lines with abnormal character spacing.
  ensure(48);section("Final pre-departure checks");(r.outing.finalChecks||[]).forEach(x=>{doc.setFont("helvetica","normal");doc.setFontSize(9);doc.text(`- ${x}`,left,y);y+=5;});
